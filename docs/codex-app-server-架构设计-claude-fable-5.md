...
| 审批 pending 表 | 网关内存 + 审计日志落盘 | NFR-8：决议 100% 留痕 |
...
- **背压（NFR-6）**：JSON-RPC `-32001` → 指数退避+抖动重试（上限 5 次）→ UI 拥塞提示；桥→前端方向合并高频 delta（≥60Hz 时按帧节流）。
- **进程失效**：app-server 子进程退出 → 桥标记实例 `crashed` → 自动重启 → `thread/resume` 续接 → 信封通告；网关重启 → 前端重连 catch-up + `session:list` 重建。
- **监控**：出入站 JSON-RPC 计数/延迟、pending 审批年龄、子进程存活、`deprecationNotice`/`warning` 通知直接上报为运维事件。
