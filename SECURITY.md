...
- `admin-audit.jsonl` records host-configuration operation outcomes and missing-confirmation denials with recursively redacted summaries and upstream errors. Both audit files are owner-only; neither is tamper-evident or a non-repudiation system. Admin audit retention is not currently bounded.
